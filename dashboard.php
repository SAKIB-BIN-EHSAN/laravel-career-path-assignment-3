...
if (!isset($_SESSION['user_id'])) {
    ...
    $feedbackFormLink = 'http://localhost:8080/feedback.php/' . $_SESSION['user_id'];

    $myfile = fopen("data/feedbackData.txt", "r") or die("Unable to open file!");
    $filename = "data/feedbackData.txt";
    $feedbacks = file($filename, FILE_IGNORE_NEW_LINES);

    $currentUserFeedback = [];
    foreach ($feedbacks as $feedback) {
        $row = explode(",", $feedback);
        if ($row[0] == $_SESSION['user_id']) {
            $data = "";
            for ($i=1; $i<count($row); $i++) {
                $data .= $row[$i];
            }
            array_push($currentUserFeedback, $data);
        }
    }
...
<header class="bg-white">
    <nav class="flex items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end">
            <span class="text-sm font-semibold leading-6 text-gray-900 mr-3">
                <?= $_SESSION['user_name'] ?>
            </span>
    ...
    <div class="relative flex min-h-screen overflow-hidden bg-gray-50 py-6 sm:py-12">
        ...
        <div class="relative max-w-7xl mx-auto">
            <div class="flex justify-end">
                <span class="block text-gray-600 font-mono border border-gray-400 rounded-xl px-2 py-1">Your feedback form link: <strong><?= $feedbackFormLink; ?></strong></span>
            </div>
            <h1 class="text-xl text-indigo-800 text-bold my-10">Received feedback</h1>
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                
                <?php if (count($currentUserFeedback) !== 0): ?>
                    <?php foreach ($currentUserFeedback as $feedback): ?>
                ...
                <?php if (count($currentUserFeedback) === 0): ?>
                    <h1 class="text-gray-900"> No Feedback Found </h1>
                <?php endif; ?>
